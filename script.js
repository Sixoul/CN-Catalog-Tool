 ...
 * v2.8 - Code Cleanup & Reference Fixes
 */
...
// ==========================================
// 3. HELPER FUNCTIONS (Must be defined first)
// ==========================================
...
function renderAdminLbPreview() {
    ...
    sorted.forEach((ninja, index) => { 
        c.innerHTML += `<div class="admin-lb-preview-row"><div class="admin-lb-rank">#${index + 1}</div><div class="admin-lb-name">${ninja.name}</div><div class="admin-lb-points">${ninja.points}</div></div>`; 
    }); 
}
    ...
    if(cList){ 
        ...
        coinsData.forEach((c, index) => { 
            ...
            <div class="admin-list-wrapper">
                <div style="display:flex; flex-direction:column; margin-right:5px;">${upBtn}${downBtn}</div>
...
function updateQueueStatus(id, s) {
    if(db){
        if(s==='Picked Up') db.collection("queue").doc(id).delete();
        ...
        if(idx>-1 && queueData[idx]){
            if(s==='Picked Up') queueData.splice(idx,1);
            else queueData[idx].status=s;
            saveLocal('cn_queue',queueData);
            renderQueue();
            renderAdminLists();
        }
