 ...
 * v7.2 - Reordered for Stability (Fixes "Not Defined" Errors)
 */
...
function renderGames() {
    ...
    if (heroContainer) {
        if(activeGame) { heroContainer.innerHTML = `<div class="game-hero" style="background-image: url('${activeGame.image}');"><div class="game-hero-content"><h1>${activeGame.title}</h1><p>${activeGame.desc}</p></div></div>`; } 
        else { heroContainer.innerHTML = `<div class="game-hero" style="display:flex;align-items:center;justify-content:center;background:#111;"><h2 style="color:#666;">No Game Selected</h2></div>`; }
    }
...
function renderJams() {
    ...
    if(activeJams.length === 0) { track.innerHTML = '<div class="jam-slide active" style="display:flex; align-items:center; justify-content:center; background:#111;"><h2 style="color:#666;">No Active Events</h2></div>'; } 
        ...
        activeJams.forEach((jam, idx) => {
            const isWinnerMode = jam.status === 'revealed'; const themeColor = jam.color || '#f1c40f';
            let winnerHtml = '';
...
function renderAdminLists() { 
    renderAdminNews(); renderAdminRules(); renderAdminCoins(); renderAdminCatalog(); 
    renderAdminRequests(); renderAdminQueue(); renderAdminLbPreview(); 
    renderAdminInterest(); renderAdminJamsList(); renderAdminGames(); renderAdminChallenges(); 
}
...
function openAdminGames() { const activeGame = gamesData.find(g => g.status === 'active'); if(activeGame) { document.getElementById('ag-title').value = activeGame.title; document.getElementById('ag-image').value = activeGame.image; document.getElementById('ag-desc').value = activeGame.desc; renderAdminGameScores(activeGame); } else { document.getElementById('ag-title').value = ''; document.getElementById('ag-image').value = ''; document.getElementById('ag-desc').value = ''; document.getElementById('admin-game-scores-list').innerHTML = '<p style="color:#666;">No active game. Create one above.</p>'; } }
