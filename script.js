...
function renderGames() {
        ...
        if(activeGame && activeGame.scores && activeGame.scores.length > 0) {
            ...
            sorted.forEach((s, i) => {
                ...
                <div class="game-lb-row">
                    <div class="g-rank">#${rank}</div>
                    <div class="g-name">${formatName(s.name)}</div>
                    <div class="g-score">${s.score}</div>
                    <div class="g-prize">${prizeHtml}</div>
                </div>`;
            });
        } else {
            lbContainer.innerHTML = '<p style="color:#666; text-align:center; padding:20px;">No scores yet. Be the first!</p>';
        }
    }
    ...
    if (historyList) {
        ...
        if(pastGames.length === 0) {
            historyList.innerHTML = '<p style="color:#666; text-align:center;">No history.</p>';
        } else {
            pastGames.forEach(g => {
                historyList.innerHTML += `
                <div class="list-card" style="margin-bottom:10px; padding:15px;">
                    <div class="card-info">
                        <h3 style="font-size:0.9rem;">${g.title}</h3>
                        <p style="font-size:0.7rem;">${g.month || 'Archived'}</p>
                    </div>
                </div>`;
            });
        }
...
function renderJams() {
    ...
    if(pastJams.length === 0) {
        ...
        pastJams.forEach(jam => {
            let topWinnerName = "View Winners";
            let winnerAction = "";
            if(jam.winners && jam.winners.length > 0) {
                topWinnerName = jam.winners[0].gameTitle;
