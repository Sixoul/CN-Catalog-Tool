...
function loadCatalog() {
    ...
    if(currentUser?.isAdmin) { db.collection("requests").orderBy("createdAt", "desc").limit(50).onSnapshot(snap => { requestsData = snap.docs.map(d => ({id: d.id, ...d.data()})); renderAdminRequests(); }); }
}
function loadQueue() {
