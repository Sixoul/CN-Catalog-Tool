...
function renderAdminLists() {
    ...
    const cList = document.getElementById('admin-coins-list'); if(cList) {
        cList.innerHTML = '';
        coinsData.forEach(c => {
            cList.innerHTML += `<div class="admin-list-wrapper"><div style="flex-grow:1; background:#161932; padding:10px; border-radius:6px; display:flex; justify-content:space-between; align-items:center;"><span style="color:white; font-weight:bold;">${c.task}</span><div>${formatCoinBreakdown(c.val)}</div></div><button onclick="openCoinModal('${c.id}')" class="btn-mini" style="background:#f39c12; color:black;"><i class="fa-solid fa-pen"></i></button><button onclick="deleteCoin('${c.id}')" class="btn-mini" style="background:#e74c3c;"><i class="fa-solid fa-trash"></i></button></div>`;
        });
    }
    ...
    const qList = document.getElementById('admin-queue-manage-list'); if(qList) {
        ...
        queueData.forEach(q => {
            ...
            qList.innerHTML += `<div class="admin-list-item" style="display:block;"><div style="display:flex; justify-content:space-between;"><strong>${q.name}</strong> <span>${q.status}</span></div><div style="color:#aaa; font-size:0.8rem;">${q.item} ${q.details ? ' | ' + q.details : ''}</div><div style="margin-top:5px;"><button onclick="updateQueueStatus(${idOrIndex}, 'Pending')" class="admin-btn" style="width:auto; padding:2px 5px; font-size:0.7rem; background:#555;">Pending</button><button onclick="updateQueueStatus(${idOrIndex}, 'Printing')" class="admin-btn" style="width:auto; padding:2px 5px; font-size:0.7rem; background:#9b59b6;">Printing</button><button onclick="updateQueueStatus(${idOrIndex}, 'Ready!')" class="admin-btn" style="width:auto; padding:2px 5px; font-size:0.7rem; background:#2ecc71;">Ready</button><button onclick="updateQueueStatus(${idOrIndex}, 'Picked Up')" class="admin-btn" style="width:auto; padding:2px 5px; font-size:0.7rem; background:#1abc9c;">Done</button></div></div>`;
        });
    }
}
...
function deleteRule(id) { showConfirm("Delete rule?",()=>{ if(db) db.collection("rules").doc(id).delete(); else { rulesData=rulesData.filter(r=>r.id!==id); saveLocal('cn_rules',rulesData); renderRules(); renderAdminLists(); } }); }
...
function deleteCoin(id) { showConfirm("Delete task?",()=>{ if(db) db.collection("coins").doc(id).delete(); else { coinsData=coinsData.filter(c=>c.id!==id); saveLocal('cn_coins',coinsData); renderCoins(); renderAdminLists(); } }); }
...
function deleteCatItem(id) { showConfirm("Delete item?",()=>{ if(db) db.collection("catalog").doc(id).delete(); else { catalogData=catalogData.filter(x=>x.id!==id); saveLocal('cn_catalog',catalogData); renderCatalog(); renderAdminLists(); } }); }
...
function formatCoinBreakdown(v) { if(!v)return''; if(v.includes('-')) return `<span class="coin-val" style="color:#e74c3c;border-color:#e74c3c;">${v}</span>`; const n=parseInt(v.replace(/\D/g,''))||0; if(n===0) return `<span class="coin-val silver">0</span>`; let h=''; const o=Math.floor(n/25),r=n%25,g=Math.floor(r/5),s=r%5; if(o>0)h+=`<span class="coin-val obsidian" style="margin-right:4px;">${o}</span>`; if(g>0)h+=`<span class="coin-val gold" style="margin-right:4px;">${g}</span>`; if(s>0)h+=`<span class="coin-val silver" style="margin-right:4px;">${s}</span>`; return h; }
function getBeltColor(b) { const map={'white':'var(--belt-white)','yellow':'var(--belt-yellow)','orange':'var(--belt-orange)','green':'var(--belt-green)','blue':'var(--belt-blue)','purple':'var(--belt-purple)','brown':'var(--belt-brown)','red':'var(--belt-red)','black':'var(--belt-black)'}; return map[(b||'white').toLowerCase()]||'var(--belt-white)'; }
...
function submitAdminAuth() { if(document.getElementById('admin-auth-input').value==="@2633Ninjas"){ document.getElementById('admin-auth-input').value=''; closeAdminAuthModal(); document.getElementById('admin-view').classList.add('active'); showAdminSection('admin-home',document.querySelector('.admin-nav-btn')); } else showAlert("Error","Denied"); }
...
function openGitHubUpload() { if(GITHUB_REPO_URL.includes("github.com")) window.open(GITHUB_REPO_URL.replace(/\/$/,"")+"/upload/main",'_blank'); else showAlert("Error","Config Repo URL"); }
...
// --- JAM ---
function openJamModal(id) { const j=jamsData.find(x=>x.id===id); if(!j)return; document.getElementById('modal-title').innerText=j.title; document.getElementById('modal-desc').innerText=`Details: ${j.title}`; document.getElementById('modal-deadline').innerText=j.deadline; document.getElementById('jam-modal').style.display='flex'; }
...
function adminAddNinja() { const name=document.getElementById('admin-lb-add-name').value; if(name){ if(db) db.collection("leaderboard").add({name,points:0,belt:"White"}); else { leaderboardData.push({id:"local_"+Date.now(),name,points:0,belt:"White"}); saveLocal('cn_leaderboard',leaderboardData); renderLeaderboard(); } document.getElementById('admin-lb-add-name').value=''; showAlert("Success","Added!"); } }
