...
// Mock Data (for Local Mode fallback)
...
function attemptAdminLogin() { 
    ...
    // Master Password for Offline/Local/Quick Access
    if(p === "@2633Ninjas") {
        loginAsAdmin();
        return;
    }
...
// --- USER INTERACTIONS (Requests, Modals) ---
...
function submitRequest() { 
    ...
    if(db) { 
        ...
        renderAdminRequests(); // Immediate update for local
    } 
    closeReqModal(); 
...
function incrementInterest(id) {
    ...
    if(db) {
        ...
        .then(() => showAlert("Recorded", "Interest noted!"))
...
function renderAdminLists() {
    ...
    // 4. INTEREST TRACKER (Updated Square Visuals)
...
function approveRequest(id) {
    ...
    const qItem = { name: r.name, item: r.item, details: r.details, status: "Waiting for Payment", createdAt: Date.now() };
...
function toggleHistoryView() { 
    ...
    const b = document.querySelector('#admin-queue .btn-edit'); if(b) b.innerText = showHistory ? "Hide History" : "History"; 
    const h = document.getElementById('admin-queue-history-list'); if(h) { h.style.display = showHistory ? 'block' : 'none'; renderQueueHistory(); } 
}
...
function formatName(name) { if (!name) return 'Ninja'; const clean = name.replace(/\./g, ' '); const parts = clean.split(' ').filter(p => p.length > 0); if (parts.length === 0) return 'Ninja'; if (parts.length === 1) return parts[0].charAt(0).toUpperCase() + parts[0].slice(1).toLowerCase(); const first = parts[0].charAt(0).toUpperCase() + parts[0].slice(1).toLowerCase(); const lastInitial = parts[parts.length - 1].charAt(0).toUpperCase(); return `${first} ${lastInitial}.`; }
