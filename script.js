...
function renderQueueHistory() {
    ...
    if(past.length===0) h.innerHTML='<p style="color:#666; font-size:0.8rem;">No history.</p>';
    else past.forEach(q => h.innerHTML += `<div class="admin-list-item" style="opacity:0.6"><strong>${q.name}</strong> - ${q.item} <span style="font-size:0.7rem">${q.createdAt ? new Date(q.createdAt).toLocaleDateString() : 'N/A'}</span></div>`);
}
...
function renderQueue() {
     ...
     if(q.length===0) c.innerHTML='<p style="color:#666;text-align:center;">Empty.</p>';
         ...
         c.innerHTML += `<div class="${cardClass}"><div class="q-left"><div class="q-number">${x+1}</div><div class="q-info"><h3>${i.name}</h3><p>${i.item}</p></div></div><div class="q-status ${cl}">${i.status} <i class="fa-solid ${icon}"></i></div></div>`;
     });
}
...
function renderAdminLists() {
    ...
    const catList = document.getElementById('admin-cat-list'); if(catList) {
        ...
        if(st.length>0) { catList.innerHTML+=`<div class="admin-tier-header">Interest Tracker</div>`; st.forEach(s=>catList.innerHTML+=`<div class="interest-item"><span>${s.name}</span><span class="interest-count">${s.interest||0}</span></div>`); }
        ...
        tiers.forEach(tier => {
            ...
            items.forEach(i => {
                ...
                <div class="admin-store-card ${hiddenClass}">
                    ...
                    <div style="flex-grow:1;"><h4 style="margin:0; color:white; font-size:0.9rem;">${i.name}</h4><p style="margin:0; color:#888; font-size:0.8rem;">${i.cost}</p></div>
